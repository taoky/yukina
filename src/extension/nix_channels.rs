...
use crate::{Cli, NormalizedFileStats, NormalizedVoteItem};
use anyhow::Result;

#[derive(Debug)]
pub struct NixChannels {
    valid_narurl_regex: regex::Regex,
}
...
impl NixChannels {
    fn narinfo_parse(&self, contents: &str) -> Result<(String, u64)> {
        ...
        // Check if url is valid -- to avoid being tricked by malicious narinfo files
        if !self.valid_narurl_regex.is_match(&url) {
            return Err(anyhow::anyhow!("Invalid nar URL"));
        }
        Ok((url, size))
    }
}
...
impl Extension for NixChannels {
    ...
    fn parse_downloaded_file(
        ...
        let (narurl, narsize) = self.narinfo_parse(&narinfo)?;
        if narsize > args.filesize_limit {
            return Ok(None);
        }
        let new_item = NormalizedVoteItem {
            path: narurl,
            stats: NormalizedFileStats {
                // Here we give the same normalized score as the original score
                // as narinfo file is useless without the nar file
...
Sig: cache.nixos.org-1:k2k4/lzGGv2pgRbpSGI2sAfAq6/D+2cGCggJ7EikGpkYIftD1ss23gQmNQ7jMZP+qDhG5JPVu/bc/kBqALsgCQ=="#;
        let (url, size) = ext.narinfo_parse(contents).unwrap();
        assert_eq!(
            url,
